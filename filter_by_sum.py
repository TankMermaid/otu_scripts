...
if __name__ == '__main__':
	p = argparse.ArgumentParser(description='filter otus based on their total abundance')
	p.add_argument('table', help='input otu table')
	p.add_argument('out', help='output otu table')
	g = p.add_mutually_exclusive_group(required=True)
	g.add_argument('-l', '--log', type=float, help='minimum (negative log10)')
	g.add_argument('-m', '--min', type=float, help='minimum (raw value)')
	args = p.parse_args()
	...
	criterion = (np.sum, lambda x,y: x>y, args.min)
